...
static struct bme68x_conf bme68x_config;
...
return_values_init bsec_iot_init(float sample_rate, float temperature_offset, bme68x_write_fptr_t bus_write,
                    bme68x_read_fptr_t bus_read, state_load_fct state_load, config_load_fct config_load)
{
...
void bme680_bsec_trigger_measurement(bsec_bme_settings_t *sensor_settings)
{
    ...
    int8_t bme680_status = BME68X_OK;
...
//        bme68x_config.odr = BME68X_ODR_NONE; // XXX
        ...
        rslt = bme68x_set_conf(&bme68x_config, &bme68x_g);

        /* Set the remaining gas sensor settings and link the heating profile */
        heatr_conf.enable = sensor_settings->run_gas;
        /* Create a ramp heat waveform in 3 steps */
        heatr_conf.heatr_temp = sensor_settings->heater_temperature; /* degree Celsius */
        heatr_conf.heatr_dur = sensor_settings->heating_duration; /* milliseconds */
        rslt = bme68x_set_heatr_conf(BME68X_FORCED_MODE, &heatr_conf, &bme68x_g);

        /* Set power mode as forced mode and trigger forced mode measurement */
        rslt = bme68x_set_op_mode(BME68X_FORCED_MODE, &bme68x_g);

        /* Get the total measurement duration so as to sleep or wait till the measurement is complete */
        meas_period = bme68x_get_meas_dur(BME68X_FORCED_MODE, &bme68x_config, &bme68x_g); // + (heatr_conf.heatr_dur * 1000);
    ...
    uint8_t opmode = BME68X_FORCED_MODE;
    bme680_status = bme68x_get_op_mode(&opmode, &bme68x_g);
    ...
    int32_t bsec_process_data)
{
    ...
    {
        bme680_status = bme68x_get_data(BME68X_FORCED_MODE, &data, &n_fields, &bme68x_g);
 ...
 * @return      none
 */
void bsec_iot_loop(get_timestamp_us_fct get_timestamp_us, output_ready_fct output_ready,
                    state_save_fct state_save, uint32_t save_intvl)
{
    ...
    while (1)
    {
        ...
        time_stamp = (uint64_t)get_timestamp_us() * 1000;
    	...
    	time_stamp_interval_ms = ((uint64_t)sensor_settings.next_call - (uint64_t)get_timestamp_us() * 1000) / 1000000;
